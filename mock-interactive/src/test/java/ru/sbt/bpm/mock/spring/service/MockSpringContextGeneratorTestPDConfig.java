...
public class MockSpringContextGeneratorTestPDConfig extends AbstractTestNGSpringContextTests {
	...
	public void testGettingData () throws JAXBException, IOException, TransformerException, XMLStreamException {
		...
		assertEquals("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
		             ...
		             + "   </bean>\n"
		             + "   <ns2:channel id=\"javaCompEnvJmsQLegaLToCrmorgSyncreSp_channel\"/>\n"
		             + "   <ns2:channel id=\"javaCompEnvJmsQLegaLFromCrmorgSyncreSp_channel\"/>\n"
		             + "   <ns3:inbound-gateway request-destination=\"javaCompEnvJmsQLegaLToCrmorgSyncreSp_queue\"\n"
		             + "                        "
		             ...
		             + "\"/>\n"
		             + "   <ns2:channel id=\"javaCompEnvJmsQLegaLFromCrmorg_channel\"/>\n"
		             + "   <ns2:channel id=\"javaCompEnvJmsQLegaLToCrmorg_channel\"/>\n"
		             + "   <ns3:outbound-gateway id=\"javaCompEnvJmsQLegaLCf_jmsout\"\n"
		             ...
		             + "   <ns2:channel id=\"javaCompEnvJmsQLegaLFromCrmorgSyncreSp_routerChannel\"/>\n"
		             + "   <ns2:router input-channel=\"MockOutboundRouterResponse\"\n"
		             + "               expression=\"payload.jmsConnectionFactoryName+'_'+payload.queue\"\n"
		             + "               id=\"MockRouter\">\n"
		             + "      <ns2:mapping value=\"java:comp/env/jms/Q.LEGAL.CF_java:comp/env/jms/Q.LEGAL.FROM.CRMORG"
		             + ".SYNCRESP\"\n"
		             + "                   channel=\"javaCompEnvJmsQLegaLFromCrmorgSyncreSp_routerChannel\"/>\n"
		             + "   </ns2:router>\n"
		             + "   <ns2:service-activator output-channel=\"java:comp/env/jms/Q.LEGAL.FROM.CRMORG.SYNCRESP\"\n"
		             + "                          input-channel=\"javaCompEnvJmsQLegaLFromCrmorgSyncreSp_routerChannel"
		             + "\"\n"
		             + "                          expression=\"payload.payload\"/>\n"
		             + "   <ns2:channel id=\"javaCompEnvJmsQLegaLFromCrmorg_routerChannel\"/>\n"
		             + "   <ns2:router input-channel=\"DriverRouterChannelInput\"\n"
		             + "               expression=\"payload.jmsConnectionFactoryName+'_'+payload.queue\"\n"
		             + "               id=\"DriverRouter\">\n"
		             + "      <ns2:mapping value=\"java:comp/env/jms/Q.LEGAL.CF_java:comp/env/jms/Q.LEGAL.FROM"
		             + ".CRMORG\"\n"
