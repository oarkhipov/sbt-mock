<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[local-name()=$xsdTagsToImport]" mode="base">
        ...
        <xsl:if test="$DEBUG">
            <xsl:comment>local-name=$xsdTagsToImport base - <xsl:value-of select="./local-name()"/>  <xsl:value-of select="concat($targetNS,'-',$parrentNS)"/></xsl:comment>
        </xsl:if>
    ...
    <xsl:template match="xsd:complexType" mode="template" priority="2">
        ...
        <xsl:if test="$DEBUG">
            <xsl:comment>xsd:complexType - template</xsl:comment>
        </xsl:if>
    ...
    <xsl:template match="xsd:element[@ref]" mode="Inside"  priority="4">
        ...
        <xsl:if test="$DEBUG">
            <xsl:comment>xsd:element[@ref]- inside <xsl:value-of select="''"/> </xsl:comment>
        </xsl:if>
        <xsl:element name="xsl:apply-templates">
