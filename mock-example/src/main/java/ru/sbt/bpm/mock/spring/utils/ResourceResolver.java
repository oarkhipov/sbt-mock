...
public class ResourceResolver implements LSResourceResolver {
     ...
     * пока делает только одну ьпростую вещь - берет baseURI (адрес точки вызова), отризает от него путь, присоединяет к нему systemId(имя вызываемого файла). Создает объект, который возвращает потоки к этомук ресурсу.
