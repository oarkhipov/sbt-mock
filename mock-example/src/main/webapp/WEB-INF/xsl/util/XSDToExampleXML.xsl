<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="digitTypes" select="tokenize(concat('int ',$xsdNsAlias,':int xsd:int integer ',$xsdNsAlias,':integer xsd:integer long ',$xsdNsAlias,':long xsd:long double xsd:double float ',$xsdNsAlias,':float xsd:float decimal ',$xsdNsAlias,':decimal xsd:decimal'),'\s+')"/>
    ...
    <!--сложный тип-->
    <xsl:template match="xsd:complexType" mode="importedType" priority="1001">
    ...
    <!--enumeration-->
    <xsl:template match="xsd:simpleType[./xsd:restriction/xsd:enumeration]" mode="importedType" priority="100">
        <xsl:param name="params" select="./xsd:restriction/*"/>
        <xsl:variable name="insideParams" select="($params | ./xsd:restriction/*[not(./name()=$params/name())])"/>
    ...
    <xsl:template match="xsd:simpleType[mock:getNamespaceAlias(./xsd:restriction/@base)=parent::*/namespace::*/local-name()
                                ...
                                ] " mode="importedType" priority="1000">
        <xsl:param name="params" select="./xsd:restriction/*"/>
        <xsl:variable name="insideParams" select="($params | ./xsd:restriction/*[not(./name()=$params/name())])"/>
