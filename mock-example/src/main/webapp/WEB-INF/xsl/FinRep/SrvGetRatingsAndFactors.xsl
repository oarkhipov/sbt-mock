...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="rsd:ratingPeriod">
		<rs:ratingPeriod>
			...
			<rs:listOfCardinalFactor>
				...
				<xsl:apply-templates select="./rsd:listOfCardinalFactor/rsd:cardinalFactor"/>
			</rs:listOfCardinalFactor>
		</rs:ratingPeriod>
	</xsl:template>
	...
	<xsl:template match="rsd:ratingSupport">
		<rs:ratingSupport>
			...
			<rs:listOfSupportFactor>
				<!--1 or more repetitions:-->
				<xsl:apply-templates select="./rsd:listOfSupportFactor/rsd:supportFactor"/>
			</rs:listOfSupportFactor>
		</rs:ratingSupport>
	</xsl:template>
		...
		<rs:ratingCalc>
			...
			<xsl:if test="./rsd:listOfRatingAjustment">
				<rs:listOfRatingAjustment>
					...
					<xsl:apply-templates select="./rsd:listOfRatingAjustment/rsd:ratingAjustment"/>
				</rs:listOfRatingAjustment>
			</xsl:if>
			<!--Optional:-->
			...
			<xsl:if test="./rsd:listOfResultRating">
				<rs:listOfResultRating>
					<!--Zero or more repetitions:-->
					<xsl:apply-templates select="./rsd:listOfResultRating/rsd:resultRating"/>
				</rs:listOfResultRating>
			</xsl:if>
			<!--Optional:-->
			<xsl:if test="./rsd:listOfCalculatedFactor">
				<rs:listOfCalculatedFactor>
					<!--Zero or more repetitions:-->
					<xsl:apply-templates select="./rsd:listOfCalculatedFactor/rsd:calculatedFactor"/>
				</rs:listOfCalculatedFactor>
			</xsl:if>
			<!--Optional:-->
			<xsl:if test="./rsd:listOfFixedFactor">
				<rs:listOfFixedFactor>
					<!--1 or more repetitions:-->
					<xsl:apply-templates select="./rsd:listOfFixedFactor/rsd:fixedFactor"/>
				</rs:listOfFixedFactor>
			</xsl:if>
			<!--Optional:-->
			<xsl:if test="./rsd:listOfRatingPeriod">
				<rs:listOfRatingPeriod>
					<!--1 or more repetitions:-->
					<xsl:apply-templates  select="./rsd:listOfRatingPeriod/rsd:ratingPeriod"/>
				</rs:listOfRatingPeriod>
			</xsl:if>
			<!--Optional:-->
			<xsl:if test="./rsd:listOfRatingSupport">
				<rs:listOfRatingSupport>
					<!--Zero or more repetitions:-->
					<xsl:apply-templates select="./rsd:listOfRatingSupport/rsd:ratingSupport"/>
				</rs:listOfRatingSupport>
			</xsl:if>
		</rs:ratingCalc>
	</xsl:template>
	...
	<xsl:template match="rsd:rating">
		<rs:rating>
			...
			<rs:listOfRatingCalc>
				<!--1 or more repetitions:-->
				<xsl:apply-templates select="./rsd:listOfRatingCalc/rsd:ratingCalc"/>
			</rs:listOfRatingCalc>
		</rs:rating>
	</xsl:template>
	...
	<xsl:template name="FinAnalysis">
		...
		<crm:getRatingsAndFactorsRs>
			<rs:listOfRating>
				<!--1 or more repetitions:-->
				<xsl:apply-templates select="$data/rsd:response[@name=$response]//rsd:listOfRating/rsd:rating"/>
			</rs:listOfRating>
		</crm:getRatingsAndFactorsRs>
	</xsl:template>
