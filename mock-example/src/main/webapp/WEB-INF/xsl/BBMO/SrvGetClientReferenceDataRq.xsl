...
<xsl:param name="correlation-id" select="''"/>
   ...
   <xsl:template match="/">
      ...
      <xsl:element xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" name="soap:Envelope">
         <xsl:call-template name="KD4Header">
            ...
            <xsl:with-param name="operation-name" select="string('SrvGetClientReferenceDataRq')"/>
         ...
         <soap:Body>
            <xsl:call-template name="SrvGetClientReferenceDataRq">
   ...
   <xsl:template match="rsd:SrvGetClientReferenceDataRq">
      <tns:SrvGetClientReferenceDataRq/>
   </xsl:template>
   ...
   <xsl:template name="SrvGetClientReferenceDataRq">
      <xsl:param name="request"/>
      <xsl:param name="data"/>
      <xsl:element name="tns:SrvGetClientReferenceDataRq">
         <xsl:if test="$data/rsd:request[@name=$request]/rsd:ClientId">
            <tns:ClientId>
               <xsl:value-of select="$data/rsd:request[@name=$request]/rsd:ClientId"/>
            </tns:ClientId>
         </xsl:if>
         <xsl:apply-templates select="$data/rsd:request[@name=$request]/rsd:BankSubdivision"/>
         <xsl:if test="$data/rsd:request[@name=$request]/rsd:TaxId">
            <tns:TaxId>
               <xsl:value-of select="$data/rsd:request[@name=$request]/rsd:TaxId"/>
            </tns:TaxId>
         </xsl:if>
         <xsl:if test="$data/rsd:request[@name=$request]/rsd:TaxRegReasonCode">
            <tns:TaxRegReasonCode>
               <xsl:value-of select="$data/rsd:request[@name=$request]/rsd:TaxRegReasonCode"/>
            </tns:TaxRegReasonCode>
         </xsl:if>
         <xsl:if test="$data/rsd:request[@name=$request]/rsd:AgreementIdentifier">
            <tns:AgreementIdentifier>
               <xsl:value-of select="$data/rsd:request[@name=$request]/rsd:AgreementIdentifier"/>
            </tns:AgreementIdentifier>
         </xsl:if>
      </xsl:element>
   </xsl:template>
