...
<xsl:output method="xml" indent="yes" omit-xml-declaration="yes"/>
   <xsl:param name="name"
              select="//*[local-name()='Envelope' and namespace-uri()='http://schemas.xmlsoap.org/soap/envelope/']/*[local-name()='Body' and namespace-uri()='http://schemas.xmlsoap.org/soap/envelope/']/*[local-name()='SrvPermitAccountChargeOperationRq' and namespace-uri()='http://sbrf.ru/prpc/bbmo/10']/*[local-name()='AccountNumber' and namespace-uri()='http://sbrf.ru/prpc/bbmo/10']/text()"/>
   <xsl:param name="dataFileName"
              select="'../../data/BBMO/xml/SrvPermitAccountChargeOperationData.xml'"/>
...
<xsl:param name="correlation-id" select="''"/>
   ...
   <xsl:template match="rsd:SrvPermitAccountChargeOperationRs">
      <tns:SrvPermitAccountChargeOperationRs/>
   </xsl:template>

   <xsl:template name="SrvPermitAccountChargeOperation">
      <xsl:param name="response"/>
      <xsl:param name="data"/>
      <xsl:element name="tns:SrvPermitAccountChargeOperationRs"/>
   </xsl:template>
</xsl:stylesheet>
